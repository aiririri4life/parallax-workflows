...
| `get_peer_snapshot` | per symbol | Factor scores + peer set. `target_company` (top-level) is the name field to cross-validate — not `name`, which belongs to individual peer rows |
...
strength deserves a relative check, not just an absolute one.
