...
| `crypto-rotation-accumulator.md` + `crypto-rotation-retiree.md` | Same thesis, two `client_profile`s (27yo accumulator vs. 85yo drawing income) — the two-pass client-conditioning acceptance test (design doc §9.5): Pass-1 statuses must be identical across both runs; the holder-dependent layer and client-conditioned ranking must differ; the retiree run must be harsher and use the stronger disclaimer variant |

See the parent skill's design doc for full rationale; these files are the inputs, not the
